...
>
  <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; min-height: 160px;">
    <div style="padding: 40px 32px; border-right: 1px solid var(--rpg-border);">
      <div class="editorial-label mb-3">Mes données · Équipement</div>
    ...
    <div style="padding:40px 32px;border-right:1px solid var(--rpg-border);">
      <div class="editorial-label mb-3">Total items</div>
      <div style="font-family:var(--font-serif);font-size:2.5rem;font-weight:700;line-height:1;">
        {{ inventory.length }}
      </div>
    </div>
    <div style="padding:40px 32px;border-right:1px solid var(--rpg-border);">
      <div class="editorial-label mb-3">Équipés</div>
      <div style="font-family:var(--font-serif);font-size:2.5rem;font-weight:700;line-height:1;">
        {{ equippedCount }}
      </div>
    </div>
    <div style="padding:40px 32px;">
      <div class="editorial-label mb-3">Valeur totale</div>
      ...
      <!-- Inventaire vide -->
      <div v-if="inventory.length === 0" class="text-center py-16">
        <div class="editorial-label mb-4">Aucun item</div>
        <div style="font-family:var(--font-serif);font-size:1.5rem;font-weight:700;">Inventaire vide</div>
      ...
      <template v-else>
        ...
        <!-- ── ÉQUIPEMENT ACTIF ──────────────────────────────────────── -->
        <div v-if="equipped.length > 0" style="margin-bottom:40px;">
          <div class="editorial-label mb-4">■ Équipement actif</div>
          <v-row no-gutters style="border-top:1px solid rgba(0,0,0,0.08);">
            ...
            >
              ...
              <div v-if="item.item.statModifiers" style="font-size:12px;color:var(--rpg-ink-muted);">
                <span
        ...
        <!-- ── CONSOMMABLES ──────────────────────────────────────────── -->
        <div v-if="consumables.length > 0" style="margin-bottom:40px;">
          <div class="editorial-label mb-4">■ Consommables</div>

          <!-- Header -->
          <div style="border-bottom:1px solid rgba(0,0,0,0.08);padding-bottom:10px;margin-bottom:0;">
            <v-row no-gutters>
              <v-col cols="4"><div class="editorial-label">Item</div></v-col>
              <v-col cols="5"><div class="editorial-label">Description</div></v-col>
              <v-col cols="2"><div class="editorial-label">Effet</div></v-col>
              <v-col cols="1" class="text-right"><div class="editorial-label">Qté</div></v-col>
            </v-row>
          </div>
          ...
          >
            <v-row no-gutters align="center">
              ...
              <v-col cols="2">
                <div v-if="item.item.effectValue" style="font-size:13px;color:#1E8449;font-weight:600;">
        ...
        <div v-if="others.length > 0">
          <div class="editorial-label mb-4">■ Autres objets</div>
          <div style="border-bottom:1px solid rgba(0,0,0,0.08);padding-bottom:10px;">
            <v-row no-gutters>
              <v-col cols="4"><div class="editorial-label">Item</div></v-col>
              <v-col cols="3"><div class="editorial-label">Type</div></v-col>
              <v-col cols="3"><div class="editorial-label">Prix unitaire</div></v-col>
              <v-col cols="2" class="text-right"><div class="editorial-label">Qté</div></v-col>
            </v-row>
          </div>
          <div
          ...
          >
            <v-row no-gutters align="center">
              <v-col cols="4">
                ...
                <div v-if="item.item.statModifiers" style="font-size:12px;margin-top:2px;">
                  <span
                    v-for="(value, stat) in parsedMods(item.item.statModifiers)"
                    :key="stat"
                    style="margin-right:8px;"
