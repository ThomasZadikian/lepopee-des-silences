...
public sealed class AdvanceCombatTurnCommandHandler
    : IRequestHandler<AdvanceCombatTurnCommand, CombatSkillActionResult>
{
    ...
    public AdvanceCombatTurnCommandHandler(
    ...
    public async Task<CombatSkillActionResult> Handle(
        AdvanceCombatTurnCommand request,
        CancellationToken cancellationToken)
    {
        ...
        var resolvedEnemy = false;

        // Resolve a single enemy turn only when an enemy is up. The player's turn is
        // a no-op here (the client just hands control back to the player).
        if (combat.Status == CombatStatus.Active
            && active is not null
            && active.Side == CombatantSide.Enemy
            && !active.IsDefeated)
        {
            ...
            resolvedEnemy = resolution.WasResolved;
        ...
        return new CombatSkillActionResult(
            ...
            ActorId: active?.Id.Value ?? Guid.Empty,
            SkillKey: null,
            TargetIds: [],
            Accepted: resolvedEnemy,
            Message: null,
