...
public sealed class RunEntityConfiguration : IEntityTypeConfiguration<RunEntity>
{
    public void Configure(EntityTypeBuilder<RunEntity> builder)
    {
        ...
        builder.Property(run => run.HealingBonusPercent).HasColumnName("healing_bonus_percent").HasDefaultValue(0);
        ...
        builder.Property(run => run.KnowledgeEntriesJson).HasColumnName("knowledge_entries_json");
        builder.Property(run => run.AmbientConversationStatesJson).HasColumnName("ambient_conversation_states_json");
